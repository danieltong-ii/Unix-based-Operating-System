...
special_file_read(vnode_t *file, off_t offset, void *buf, size_t count)
{
        ...
        if(S_ISBLK(file->vn_mode)){
                ...
                return -ENOTSUP; // return if file is a block device
        } // REVOKED

        // return read from bytedev_t by calling read function from cd_ops
...
special_file_write(vnode_t *file, off_t offset, const void *buf, size_t count)
{
        ...
        if(S_ISBLK(file->vn_mode)){
                dbg(DBG_PRINT, "(GRADING2B)\n"); // NOT HIT 2B
                return -ENOTSUP; //if file is a block device
        }// REVOKED
        // return write from bytedev_t by calling write function from cd_ops
