...
sched_switch(void)
{
    ...
    intr_setipl(IPL_HIGH); // Set priority level to high

    while (sched_queue_empty(&kt_runq)) {
        intr_disable();
        intr_setipl(IPL_LOW); // Set priority level to low and wait for interrupt
        intr_wait();
        intr_setipl(IPL_HIGH); // Set priority level to high
        dbg(DBG_PRINT, "(GRADING1C)\n");
    }
    ...
    context_switch(&oldThread->kt_ctx, &curthr->kt_ctx); // Switch to the new thread's context
    intr_setipl(oldIPL); // Restore interrupt priority level to the previous value
    dbg(DBG_PRINT, "(GRADING1C)\n");
}
