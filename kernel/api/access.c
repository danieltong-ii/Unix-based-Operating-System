...
addr_perm(struct proc *p, const void *vaddr, int perm)
{
        ...
        if((vmarea = vmmap_lookup(p->p_vmmap, vfn))){
                error = 1;
...
range_perm(struct proc *p, const void *avaddr, size_t len, int perm)
{
        ...
        int error = 1;
        uint32_t terminal_frameNum = ADDR_TO_PN(PAGE_ALIGN_UP((uint32_t)avaddr + len));
        uint32_t initial_frameNum = ADDR_TO_PN(PAGE_ALIGN_DOWN(avaddr));

        for (; initial_frameNum < terminal_frameNum; initial_frameNum++) {
              if (!addr_perm(p, PN_TO_ADDR(initial_frameNum), perm)) {
