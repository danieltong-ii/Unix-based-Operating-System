...
sys_read(read_args_t *arg)
{
        ...
        error = copy_from_user(&kern_args, arg, sizeof(read_args_t));
        if(0 <= error){
                k_buf = page_alloc();
                read_bytes = do_read(kern_args.fd, k_buf, kern_args.nbytes);
                if( 0 > read_bytes){
                        page_free(k_buf);
                ...
                error = copy_to_user(kern_args.buf, k_buf, read_bytes);
                dbg(DBG_PRINT, "(GRADING3D 1)\n");
        }
        page_free(k_buf);
        dbg(DBG_PRINT, "(GRADING3D 1)\n");
...
{ 
        ...
        int k_return = copy_from_user(&write_args, arg, sizeof(write_args));
        void *buf = NULL;


        buf = page_alloc();
        int error = copy_from_user(buf, write_args.buf, write_args.nbytes); 


        int write_return = do_write(write_args.fd, buf, write_args.nbytes);
        if(0 > write_return){
                curthr->kt_errno = -write_return;
                page_free(buf);
                dbg(DBG_PRINT, "(GRADING3D 1)\n");
                return -1;
        }
...
sys_getdents(getdents_args_t *arg)
{
        ...
        dirent_t kernel_directory_entry;
        size_t num_directory_entries;
        int error_code;
        getdents_args_t kernel_arguments;

        error_code = copy_from_user(&kernel_arguments, arg, sizeof(getdents_args_t));
        num_directory_entries = (kernel_arguments.count) / sizeof(dirent_t);
        
        int directory_bytes;
        size_t total_directory_bytes = 0;
                
        if(error_code >= 0)
        {
                size_t i = 0;
                while (i < num_directory_entries && total_directory_bytes <= kernel_arguments.count) {
                  if ((directory_bytes = do_getdent(kernel_arguments.fd, &kernel_directory_entry)) < 0) {
                      curthr->kt_errno = -directory_bytes;
                      dbg(DBG_PRINT, "(GRADING3D 1)\n"); 
                      return -1;
                  }
                  ...
                  error_code = copy_to_user(kernel_arguments.dirp + i, &kernel_directory_entry, directory_bytes);
                  total_directory_bytes += directory_bytes;
                  dbg(DBG_PRINT, "(GRADING3A)\n");
                  i++;
                }
                dbg(DBG_PRINT, "(GRADING3A)\n");
