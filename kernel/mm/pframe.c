...
pframe_pin(pframe_t *pf)
{
        ...
        if (pframe_is_pinned(pf) == 0) { // Check if the page frame is not already pinned
                list_remove(&pf->pf_link); // Remove the page frame from its current list
                list_insert_tail(&pinned_list, &pf->pf_link); // Insert the page frame at the tail of the pinned list

                npinned += 1;    // Increase the count of pinned page frames
                nallocated -= 1; // Decrease the count of allocated page frames
                
                dbg(DBG_PRINT, "(GRADING3A)\n");
        }

        pf->pf_pincount += 1; // Increment the pin count of the page frame
        dbg(DBG_PRINT, "(GRADING3A)\n");
}
